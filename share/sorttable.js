...
sorttable = {
  ...
  makeSortable: function(table)
  {
    ...
    {
      var temp = headrow[i];
      tagList = headrow[i].getElementsByTagName('span');

      for (var k=0; k < tagList.length; k++)
      {
        if (tagList[k].className.search(/\bsorttable_sort_anchor\b/) != -1)
        {
          temp = tagList[k];
          break;
        }
      ...
      if (!temp.className.match(/\bsorttable_nosort\b/))
      {
        // skip this col
        mtch = temp.className.match(/\bsorttable_([a-z0-9]+)\b/);
        ...
        {
          temp.sorttable_sortfunction = sorttable["sort_"+override];
        }
        else
        {
          temp.sorttable_sortfunction = sorttable.guessType(table,i);
        }

        // make it clickable to sort
        temp.sorttable_columnindex = i;
        temp.sorttable_tbody = table.tBodies[0];
        dean_addEvent(temp,"click", function(e)
        {
          if (this.className.search(/\bsorttable_sort_anchor\b/) != -1)
          {
            columnHeader = this.parentNode;
          }
          else
          {
            columnHeader = this;
          ...
          theadrow = columnHeader.parentNode;
