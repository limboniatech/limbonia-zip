...
echo "  <h5 id=\"progress\">Unversioned Tickets (<a class=\"item\" href=\"" . $controller->generateUri('ticket', 'create') . "?Type=software&SoftwareID=$currentItem->id\">New Ticket</a>) (<a class=\"item\" href=\"" . $module->generateUri($currentItem->id, 'releases') . "\">New Version</a>)</h5>\n";
...
foreach ($currentItem->getReleaseList('roadmap') as $release)
{
  ...
  if ($bugCount > 0)
  {
    ...
    echo "  <h5 id=\"$release->version\"><a class=\"item\" href=\"" . $controller->generateUri('ticket', $release->ticketID) . "\">$currentItem->name - $release->version</a> ($sDueDate) (<a class=\"item\" href=\"" . $controller->generateUri('ticket', 'create') . "?Type=software&SoftwareID=$currentItem->id&ReleaseID=$release->id\">New Ticket</a>)</h5>
