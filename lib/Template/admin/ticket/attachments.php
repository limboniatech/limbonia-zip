...
{
  echo "<div><a target=\"_blank\" href=\"{$attachment['link']}\" onClick=\"window.open('{$attachment['link']},'attachment$key'); return false;\">{$attachment['name']}</a> (uploaded {$attachment['time']}) <a class=\"item\" href=\"" . $module->generateUri($currentItem->id, $method, $attachment['id'], 'delete') . "\">delete</a></div>\n";
}
